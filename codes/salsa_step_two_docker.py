...
def run_inference(scan_filename: str, folder: str):
    ...
    """
    # Get nnUNet results path from environment variable
    nnUNet_results = os.environ.get("NNUNET_RESULTS", "/app/models")
    ...
    if (
        os.path.exists(os.path.join(folder, folder_name, "SALSA_segmentations_3dl"))
        == False
    ):
    ...
    model_path_3dl = os.path.join(
        nnUNet_results, "Dataset001_TALES/nnUNetTrainer__nnUNetPlans__3d_lowres"
    )
    model_path_3dc = os.path.join(
        nnUNet_results,
        "Dataset001_TALES/nnUNetTrainer__nnUNetPlans__3d_cascade_fullres",
    )

    if not os.path.exists(model_path_3dl):
        raise FileNotFoundError(
            f"Model not found at {model_path_3dl}. Please ensure models are mounted correctly."
        )
    if not os.path.exists(model_path_3dc):
        raise FileNotFoundError(
            f"Model not found at {model_path_3dc}. Please ensure models are mounted correctly."
        )
...
def adjust_shape_to_match(scan, mask):
    ...
    mask = mask[
        : min(scan_shape[0], mask_shape[0]),
        : min(scan_shape[1], mask_shape[1]),
        : min(scan_shape[2], mask_shape[2]),
    ]
...
def postprocess_case(case):
    ...
    transform_spacing = Spacing(pixdim=(1 / spx, 1 / spy, 1 / spz), mode="nearest")
    ...
    assert mask_dims[1:] == list(
        mask111.shape
    ), "There is some mismatch among the shapes of the predicted mask and the target shape extracted from the liver mask."
    ...
    mask_dims[1:] = [
        min(mask_dims[i], full_mask.shape[i] - corner[i])
        for i in range(1, len(mask_dims))
    ]
    full_mask[
        :,
        corner[1] : corner[1] + mask_dims[1],
        corner[2] : corner[2] + mask_dims[2],
        corner[3] : corner[3] + mask_dims[3],
    ] = torch.tensor(mask111.get_fdata())[
        : mask_dims[1], : mask_dims[2], : mask_dims[3]
    ]

    # Finally, we reshape the full mask from the 111 space to the original one
    transform_spacing = Spacing(pixdim=(spx, spy, spz), mode="nearest")
    mask_reshaped = transform_spacing(full_mask).squeeze(0)
    if not mask_reshaped.shape == scan.shape:
        print(
            f"There's a mismatch among the reshaped mask shape {mask_reshaped.shape} and the scan shape {scan.shape}."
        )
...
def niigz_to_segnrrd(path_mask, folder, scan_filename):
    ...
    os.remove(path_mask)
    print(
        f"----------------------------------Final mask saved as {path_mask2}----------------------------------"
    )
    ...
    if file_extension == "csv":
        ...
        for value in df["PATHS"]:
            ...
            case = {
                "scan": scan_path,
                "mask": os.path.join(
                    folder_dir, folder_name, "SALSA_segmentations_3dc", "scan.nii.gz"
                ),
